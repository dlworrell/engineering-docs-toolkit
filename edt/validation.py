...
class ValidationReport:
    ...
    def to_markdown(self) -> str:
        ...
        if not self.findings:
            ...
            return "\n".join(lines) + "\n"
        lines.append(
            "| Severity | Rule | Category | Page | Node | Message |"
        )
        lines.append("|---|---|---|---:|---|---|")
        for finding in self.findings:
            ...
            lines.append(
                ...
                f"{finding.category} | {page} | {node} | {message} |"
            )
        return "\n".join(lines) + "\n"
...
def _node_number(node: dict[str, Any]) -> str:
    ...
    if not isinstance(metadata, dict):
        return ""
    return str(
        metadata.get("number", metadata.get("equation_number", ""))
    ).strip()
...
def _validate_duplicate_ids(
    ...
    nodes: list[tuple[dict[str, Any], int | None]],
) -> None:
        ...
        if not node_id.startswith("page-"):
            report.add(
                ValidationFinding(
                    ...
                    "warning",
                    "structure",
                    "Page node id does not follow page-N format.",
                    node_id=node_id,
                )
            )
            continue
        try:
            ...
            report.add(
                ValidationFinding(
                    ...
                    "warning",
                    "structure",
                    "Page node id does not contain a valid page number.",
                    node_id=node_id,
                )
            )
    missing_pages = (
        set(range(min(page_numbers), max(page_numbers) + 1))
        - set(page_numbers)
        if page_numbers
        else set()
    )
    for page in sorted(missing_pages):
        report.add(
            ValidationFinding(
                ...
                "error",
                "structure",
                f"Missing page {page} in document page sequence.",
                page=page,
            )
        )


def _validate_theorem_proof_pairs(
    report: ValidationReport,
    nodes: list[tuple[dict[str, Any], int | None]],
) -> None:
    linear = [
        (node, page)
        for node, page in nodes
        if str(node.get("kind", "")) not in {"document", "page"}
    ]
    for index, (node, page) in enumerate(linear):
        ...
        if kind == "theorem" and (
            index + 1 >= len(linear)
            or str(linear[index + 1][0].get("kind", "")) != "proof"
        ):
            report.add(
                ValidationFinding(
                    "SEM001",
                    "warning",
                    "semantic",
                    "Theorem is not immediately followed by a proof.",
                    node_id=node_id,
                    page=page,
                )
            )
        if kind == "proof" and (
            index == 0
            or str(linear[index - 1][0].get("kind", "")) != "theorem"
        ):
            report.add(
                ValidationFinding(
                    "SEM002",
                    "warning",
                    "semantic",
                    "Proof is not immediately preceded by a theorem.",
                    node_id=node_id,
                    page=page,
                )
            )
...
def _validate_duplicate_numbers(
    report: ValidationReport,
    nodes: list[tuple[dict[str, Any], int | None]],
) -> None:
    rules = {
        ...
        if key in seen:
            report.add(
                ValidationFinding(
                    ...
                    "warning",
                    "semantic",
                    f"Duplicate {kind} number: {number}",
                    node_id=node_id,
                    page=page,
                )
            )
...
def _has_caption_child(node: dict[str, Any]) -> bool:
    ...
    return isinstance(children, list) and any(
        isinstance(child, dict) and child.get("kind") == "caption"
        for child in children
    )
        ...
        if isinstance(children, list):
            for child in children:
                if (
                    ...
                    and child.get("kind") == "caption"
                    and kind not in valid_caption_owners
                ):
                    report.add(
                        ValidationFinding(
                            "SEM022",
                            "warning",
                            "semantic",
                            "Caption is attached to a node that cannot own captions.",
                            node_id=str(child.get("id", "")),
                            page=page,
                        )
                    )
...
def _validate_references(
            ...
            if target_id not in node_ids:
                report.add(
                    ValidationFinding(
                        ...
                        "warning",
                        "reference",
                        f"Reference target does not exist: {target_id}",
                        node_id=node_id,
                        page=page,
                    )
                )
    ...
    for node_id, count in incoming.items():
        ...
        if (
            count == 0
            and kind in REFERENCE_EXPECTED
            ...
            report.add(
                ValidationFinding(
                    ...
                    "warning",
                    "reference",
                    f"{kind.title()} is never referenced.",
                    node_id=node_id,
                    page=page,
                )
            )
    ...
    if not isinstance(root, dict):
        report.add(
            ValidationFinding(
                ...
                "error",
                "structure",
                "Document EDOM has no root node.",
            )
        )
        return report
    if root.get("kind") != "document":
        report.add(
            ValidationFinding(
                ...
                "error",
                "structure",
                "Root node is not a document node.",
    ...
    if not isinstance(children, list) or not children:
        report.add(
            ValidationFinding(
                ...
                "warning",
                "structure",
                "Document has no content children.",
                node_id=str(root.get("id", "")),
            )
        )
